...
export default function Home() {
  ...
  const handleSubmitPublicReview = async () => {
    if (!publicReviewName.trim() || !publicReviewComment.trim() || publicReviewSaving) return;
    ...
    try {
      ...
      const result = (await response.json()) as { success?: boolean; review?: Review; error?: string };
      if (!response.ok) {
        setPublicReviewError(result.error || "Değerlendirmen şu anda gönderilemedi.");
        return;
      }
  ...
  return (
    ...
    >
      {publicReviewOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm" onClick={() => setPublicReviewOpen(false)}>
          <div className="w-full max-w-md rounded-3xl border border-[#c9a35b]/25 bg-[#101010] p-5 shadow-2xl md:p-6" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-[10px] font-semibold tracking-[0.3em] text-[#c9a35b]">DEĞERLENDİRME YAP</p>
                <h3 className="mt-2 text-2xl font-bold">Deneyimini paylaş</h3>
              </div>
              <button type="button" onClick={() => setPublicReviewOpen(false)} className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 text-white/50 transition hover:border-white/25 hover:text-white">×</button>
            </div>

            {publicReviewSent ? (
              <div className="mt-6">
                <div className="rounded-2xl border border-[#c9a35b]/25 bg-[#c9a35b]/10 px-4 py-4 text-sm text-[#c9a35b]">Teşekkürler. Değerlendirmen yayınlandı. ✓</div>
                <button type="button" onClick={() => { setPublicReviewOpen(false); setPublicReviewSent(false); }} className="mt-4 w-full rounded-xl bg-[#c9a35b] py-3 font-bold text-black transition hover:bg-[#dfbd76]">Kapat</button>
              </div>
            ) : (
              <>
                <input value={publicReviewName} onChange={(event) => setPublicReviewName(event.target.value)} maxLength={60} placeholder="Ad Soyad" className="mt-6 w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white outline-none placeholder:text-white/25 focus:border-[#c9a35b]/50" />
                <div className="mt-4">
                  <p className="text-xs text-white/35">Puanın</p>
                  <div className="mt-2 flex gap-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button key={star} type="button" onClick={() => setPublicReviewRating(star)} className={`text-3xl transition ${star <= publicReviewRating ? "text-[#c9a35b]" : "text-white/15"}`}>★</button>
                    ))}
                  </div>
                </div>
                <textarea value={publicReviewComment} onChange={(event) => setPublicReviewComment(event.target.value)} maxLength={300} rows={4} placeholder="Değerlendirmeni yaz..." className="mt-4 w-full resize-none rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white outline-none placeholder:text-white/25 focus:border-[#c9a35b]/50" />
                {publicReviewError && <p className="mt-2 text-xs text-red-300">{publicReviewError}</p>}
                <button type="button" onClick={handleSubmitPublicReview} disabled={!publicReviewName.trim() || !publicReviewComment.trim() || publicReviewSaving} className={`mt-4 w-full rounded-xl py-3 font-bold transition ${publicReviewName.trim() && publicReviewComment.trim() && !publicReviewSaving ? "bg-[#c9a35b] text-black hover:bg-[#dfbd76]" : "cursor-not-allowed bg-white/5 text-white/20"}`}>{publicReviewSaving ? "Gönderiliyor..." : "Değerlendirmeyi Gönder"}</button>
              </>
            )}
          </div>
        </div>
